...
## Development Lessons
- **Process Management**: Use `npx vite` (no `--port`) directly instead of `npm run dev` to ensure clean process termination when the agent stops. Avoids orphans caused by `npm` shell wrappers.
