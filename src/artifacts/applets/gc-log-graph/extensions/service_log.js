...
const ServiceLogExtension = {
  ...
  parse(line) {
    ...
    const offset = GCGraphConfig?.detectedLogTimezone || '';
  ...
  finish() {
    ...
    const config = GCGraphConfig?.serviceLog || { windowSize: 30 };
  ...
  render(chartGroup, scales, dims) {
    ...
    const { x } = scales;
    const config = GCGraphConfig?.serviceLog || {};
    ...
    this._events.forEach(d => {
      ...
      const timeStr = formatTimestampInTz(d.timestampRaw ? d.timestamp : d.endTime, d.timestampRaw || d.endTimeRaw);
      const durStr = formatDurationHuman(d.processingTime, 'ms');
      let goodsStr = '';
      if (d.lastGoodsCount !== null) {
        const human = formatCountHuman(d.lastGoodsCount);
      ...
      .on("click", function (event, d) {
        ...
        const popupContent = `
            ...
            <div style="margin-bottom: 10px; color: ${CONST.popup.textColor};">
              ...
              <strong>${d.timestampRaw ? 'Start Time' : 'End Time'}:</strong> ${formatTimestampInTz(d.timestampRaw ? d.timestamp : d.endTime, d.timestampRaw || d.endTimeRaw)}<br/>
              <strong>Processing Time:</strong> ${formatDurationHuman(d.processingTime, 'ms')}
            ...
            return `<br/><strong>Last Goods Count:</strong> ${display}`;
              ...
              ${d.metrics ? `
                ...
                <br/><strong>CarrierConnectExecuteTime:</strong> ${formatDurationHuman(d.metrics.carrierConnectExecuteTime, 'ms')}
                <br/><strong>HknAgtTime:</strong> ${formatDurationHuman(d.metrics.hknAgtTime, 'ms')}
              ` : ''}
              ${hknAgtContent}
            </div>
...
export default ServiceLogExtension;
