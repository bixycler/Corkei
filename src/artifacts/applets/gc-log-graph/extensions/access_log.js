...
import GCGraphConfig, { formatResponseSize, formatDurationHuman, formatTimestampInTz } from './config.js';
...
const AccessLogExtension = {
  ...
  finish() {
    ...
    const config = GCGraphConfig ? GCGraphConfig.accessLog : { windowSize: 30 };
  ...
  render(chartGroup, scales, dims) {
    ...
    this._events.forEach(d => {
      const sizeStr = formatResponseSize(d.size);
      const bpsStr = formatResponseSize(d.Bps) + "/s";

      // Human-readable latency using global helper
      const latencyStr = formatDurationHuman(d.latency, 'μs');

      d._tooltipHtml = `<strong>Access: ${d.method} ${d.url}</strong><br/>Time: ${formatTimestampInTz(d.timestamp, d.timestampRaw)}<br/>Status: ${d.status} | Size: ${sizeStr}<br/>Latency: ${latencyStr}<br/>Rates: ${d.rps.toFixed(1)} RPS | ${bpsStr}`;
    });
      ...
      .on("click", function (event, d) {
        ...
        const popupContent = `
            ...
            <div style="margin-bottom: 10px; color: ${CONST.popup.textColor};">
              <strong>Time:</strong> ${formatTimestampInTz(d.timestamp, d.timestampRaw)}<br/>
...
${d.originalLine}
  ...
  onZoom(event) {
    ...
    const config = GCGraphConfig ? GCGraphConfig.accessLog : { metrics: ['rps'], colors: {} };
    const metrics = config.metrics || ['rps'];
...
export default AccessLogExtension;
