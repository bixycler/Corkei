      ...
      <Match when={props.position === "bottom"}>
        <label>
          ...
          <input ref={targetCoords} type="text" size={7}
            value={`${Math.round(target().x)}, ${Math.round(target().y)}`}
            onInput={e => {
        ...
        <button type="button" onClick={() => {
          ...
          if (!canvasNode) { console.error("Canvas node not found!"); return; }
          // update download filename
          let suffix = (fixedBase() ? '-fixedBase' : '') + (shadow() ? '-shadow' : '');
          downloadImage.download = `FabrikDemo-${numSegments()}-${targetCoords.value.replace(' ', '')}${suffix}.svg`
          // serialize the SVG element
          const svgString = new XMLSerializer().serializeToString(canvasNode);
